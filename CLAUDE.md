...
1. **Backend** — Google Apps Script project (the root `.gs` files + `appsscript.json`), deployed as a Web App. It is a **pure JSON API**, not an HTML-serving app: `Code.gs` is the only entry point (`doGet`/`doPost`), dispatching to one of the whitelisted functions via a lookup table (`API_ACTIONS`) and returning `ContentService` JSON. There is no session/auth layer at the transport level — `doGet`/`doPost` execute as the deploying user (`executeAs: USER_DEPLOYING` in `appsscript.json`) regardless of caller identity, which is what lets an anonymous frontend read/write the Sheet and Drive at all; the User ID/password system described below is an *application-level* identity check layered on top of this, not a replacement for it.
   ...
   - `setLineItemExclusion(requestId, lineId, excluded, reason, userId, password)` is a third per-line mutation: marks one attachment **"Not included"** (not a valid expense) without rejecting the whole request — `Amount` forced to 0, the old amount saved to `OriginalAmount`, `Excluded`=`TRUE`, `ExcludedReason` (required, server-enforced) and `ExcludedBy` (server-resolved `fullName`) stamped; `excluded=false` ("Include again") restores `OriginalAmount` and clears all four columns. Both it and `updateLineItemAmount` now share `authorizeLineEdit_` (the exact credential/role/Pending-routing/line-belongs-to-request guard chain described above, extracted unchanged) and `recomputeRequestTotalAndAppendRemark_` (from-scratch re-sum + one appended `Remarks` line). `updateLineItemAmount` refuses an excluded line ("Include this line again before editing its amount") so `OriginalAmount` can't drift; Timesheet lines can't be excluded. The four columns live at the end of `RequestLines` (`Setup.gs`), so they flow into the published `REQUEST_LINES_CSV_URL` automatically; `frontend/common.js`'s `isLineExcluded_` reads them (CSV text `"TRUE"`). The receipt modal shows "Mark as Not included" (reason input) / "Include again" under the same `isLineEditableForCurrentApprover_` gate as Save Amount; the "Not included" tag + reason render for everyone (including the employee's own My Requests), and Export CSV (`Excluded`/`ExcludedReason` columns) / Print Preview captions keep the line, labeled.
...
18. **Per-line "Not included" exclusion for Approver/Reviewer/Verifier.** User asked for a
    way to flag a single attachment that isn't part of the expenses: amount becomes 0
    automatically, with a remarks/reason. Confirmed via clarifying questions: re-include
    allowed (restores original amount), reason required, employee sees the tag + reason,
    and export keeps the line labeled rather than dropping it. See the Architecture
    section's `setLineItemExclusion` paragraph for mechanics. New `RequestLines` columns:
    `Excluded, ExcludedReason, ExcludedBy, OriginalAmount`. Frontend: `common.js`
    (`isLineExcluded_`, `buildExcludedTagHtml_`, `patchCachedLineExclusion_`, modal
    exclude/include controls, `line-excluded` row class, `opts.reopen` to re-render the
    same line after a change), `admin.js` (`setLineItemExclusion_` wired next to both
    `onSaveAmount` call sites, CSV columns, Print Preview caption), `styles.css`.
    - **Verified**: `node --check` on all touched files; a Node simulation of the real
      `RequestService.gs` against an in-memory sheet (reason required, bad credentials,
      wrong role, cross-request LineID, Timesheet refused, exclude → total drops +
      columns stamped + Remarks line, double-exclude refused, amount-edit on excluded
      line refused, include-again restores amount/total and clears columns, terminal
      status refused) and of `patchCachedLineExclusion_` — all passed. `clasp push -f` +
      `clasp deploy -i` → `@39`; live `curl` confirmed the new action responds (generic
      bad-credential error).
    - **Not yet done**: `setupSheets()` must be re-run in the Apps Script editor to add
      the columns (until then the action returns a "re-run setupSheets()" error, and
